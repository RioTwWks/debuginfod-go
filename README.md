...
Образ на **Debian bookworm** (не Alpine) — стабильнее на Astra/Ubuntu и при ограниченном доступе к CDN Alpine.

Опционально добавьте хостовый `/usr/lib/debug` в `docker-compose.yml` и `DEBUGINFOD_SCAN_PATH=/sample,/usr/lib/debug`.
