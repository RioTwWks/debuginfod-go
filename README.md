...
| HTTP API | `/buildid/*`, `/metadata`, `/healthz`, `/readyz`, `/zabbix`, `/ui/` |
...
Для dev/demo (не для production на целевых ОС):

```bash
make -C examples/sample          # демо hello + hello.debug для индексации
docker compose up --build
```

**Astra Linux / корпоративный прокси** — системный прокси не попадает в Docker build автоматически. Экспортируйте `HTTP_PROXY`/`HTTPS_PROXY` (или `source deploy/docker/ensure-proxy-env.sh`), соберите бинарник на хосте:

```bash
sudo apt-get install -y gcc libsqlite3-dev   # один раз на хосте
make -C examples/sample
make build
make docker-astra
```

Подробно: [deploy/docker/README.md](deploy/docker/README.md) (прокси, `APT_PROFILE=astra`, оффлайн).

Образ на **Debian** (не Alpine). Опционально: `APT_MIRROR`, `network: host` при build.

Полное демо с GDB/Delve: [examples/](./examples/).
...
| `DEBUGINFOD_ADMIN_KEY` | `-admin-key` | Токен `POST /admin/rescan` | `ZABBIX_KEY` |
| `DEBUGINFOD_SCAN_ENABLED` | `-scan-enabled` | Фоновая индексация | `true` |
| `DEBUGINFOD_SCAN_WEBHOOK_URL` | `-scan-webhook-url` | Webhook после scan | — |
...
Опционально: CORS (`DEBUGINFOD_CORS_ORIGINS`), rate limiting (`DEBUGINFOD_RATE_LIMIT`), Basic Auth (`DEBUGINFOD_BASIC_AUTH_*`), TLS/mTLS (`DEBUGINFOD_TLS_*`). `/healthz` и `/readyz` доступны без Basic Auth.

### Мониторинг и UI

```http
GET /healthz              → 200 ok (liveness)
GET /readyz               → 200 ready / 503 not ready (readiness)
POST /admin/rescan        → принудительный scan (X-Admin-Token или ?key=)
