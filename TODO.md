...
- [x] **`.apk` (Alpine)** — индексация ELF из apk-архивов
- [x] **`.pacman` / `.pkg.tar.zst`** — пакеты Arch Linux
- [x] **Plain tar/tar.gz/tar.xz** — каталоги с отладочными символами без обёртки deb/rpm
- [x] **Отложенное извлечение** — не кэшировать ELF при индексации, извлекать из архива по запросу
- [x] **Индексация исходников из SRPM/DSC** — не только бинарные артефакты
...
- [x] Web UI — простой дашборд: статистика индекса, поиск по build-id
