...
**Библиотеки:** `lib.so.19.1.5.2899.debug`
...
**Исполняемые (дефис):** `quik-16.0.0.10.debug`
...
Файлы с другим форматом имени пропускаются dedup (остаются на диске, в логе `dedup skip file` на уровне DEBUG).
